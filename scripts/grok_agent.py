...
def _resolve_provider():
    ...
    if os.environ.get("XAI_API_KEY", "").strip():
        return ("xai", os.environ.get("XAI_BASE_URL", "https://api.x.ai/v1").rstrip("/"),
    ...
    if os.environ.get("OPENAI_API_KEY", "").strip():
        return ("openai", os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1").rstrip("/"),
...
def main() -> int:
    if not XAI_KEY:
        log("ERROR: no API key - set XAI_API_KEY (Grok), GROQ_API_KEY (Groq) "
