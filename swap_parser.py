...
import json, sys, time, os, re, sqlite3, urllib.request, urllib.parse, urllib.error
...
def _get(url: str, tries: int = 4, timeout: int = 30):
    ...
    for i in range(tries):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "swap-parser/1.0",
...
class TonAPI:
    ...
    def jetton(self, addr: str) -> dict:
        if addr and addr not in self._jettons:
            try:    self._jettons[addr] = _get(f"{TONAPI}/jettons/{addr}")
            except Exception: self._jettons[addr] = {}
    ...
    def chart(self, token: str, currency: str = "usd", start: int = 0, end: int = 0) -> dict:
        return _get(f"{TONAPI}/rates/chart?token={token}&currency={currency}"
...
def iter_txs(node: dict):
    ...
    tx = node.get("transaction") if isinstance(node, dict) and "transaction" in node else node
...
@dataclass
class Meta:
    symbol: str = "TON"; name: str = "Toncoin"; decimals: int = TON_DECIMALS
    address: str = "native"; usd: float | None = None
    def scale(self, raw) -> float:
        return int(raw or 0) / (10 ** self.decimals)

@dataclass
class Leg:
    dex: str; token_in: Meta; token_out: Meta
    amount_in_raw: int; amount_out_raw: int
    amount_in: float; amount_out: float
    usd_in: float | None = None; usd_out: float | None = None

@dataclass
class Refund:
    op: str; amount_raw: int; amount: float; to: str; kind: str

@dataclass
class SwapAnalysis:
    tx_hash: str = ""; event_id: str = ""; timestamp: int = 0
    wallet: str = ""; dex: str = ""
    token_in: Meta | None = None; token_out: Meta | None = None
    amount_in: float = 0.0; amount_out: float = 0.0
    rate: float | None = None; rate_inverse: float | None = None
    legs: list = field(default_factory=list); hops: int = 0
    gas_bound: float = 0.0; gas_refunded: float = 0.0; gas_spent: float = 0.0
    net_ton_change: float = 0.0; net_jetton_change: dict = field(default_factory=dict)
    total_fees: float = 0.0; tx_count: int = 0
    refunds: list = field(default_factory=list)
    success: bool = True; is_scam: bool = False
    # ---- USD-оценка (исторический курс на момент свапа, не текущий)
    usd_in: float | None = None; usd_out: float | None = None
    usd_net: float | None = None; usd_rate: float | None = None
    usd_gas_bound: float | None = None; usd_gas_refunded: float | None = None
    usd_gas_spent: float | None = None; usd_total_fees: float | None = None
    price_source: str = ""; price_errors: dict = field(default_factory=dict)
    def to_dict(self):
        d = asdict(self)
        return d
...
@dataclass
class Price:
    usd: float | None = None; source: str = ""; ts: int = 0; error: str | None = None
...
class PriceOracle:
    ...
    def __init__(self, api: "TonAPI", window: int = 172800, max_skew: int = 7200):
        self.api = api; self.window = window; self.max_skew = max_skew
    ...
    def at(self, addr: str, symbol: str, ts: int) -> Price:
        k = self.key(addr, symbol); ck = (k, ts // 3600)
        if ck in self._cache: return self._cache[ck]
        p = Price()
        if ts:
            try:
                pts = (self.api.chart(k, "usd", ts - self.window, ts + self.window) or {}).get("points") or []
                if pts:
                    best = min(pts, key=lambda x: abs(int(x[0]) - ts))
                    skew = abs(int(best[0]) - ts)
                    if skew <= self.max_skew:
                        p = Price(usd=float(best[1]), source="tonapi/rates/chart", ts=int(best[0]))
                    else:
        ...
        if p.usd is None:
            try:
                ...
                r = rt.get(k) or rt.get(k.upper()) or (next(iter(rt.values())) if rt else None)
                if r and (r.get("prices") or {}).get("USD") is not None:
                    p = Price(usd=float(r["prices"]["USD"]), source="tonapi/rates(текущий)", ts=0)
                else:
...
def _meta_for(api: TonAPI, master: dict | None) -> Meta:
    ...
    return Meta(
        ...
        decimals=int(master.get("decimals") if master.get("decimals") is not None
...
def normalize(ev: dict, tr: dict, api: TonAPI) -> SwapAnalysis:
    a = SwapAnalysis()
    a.event_id = ev.get("event_id", ""); a.timestamp = int(ev.get("timestamp") or 0)
    a.is_scam = bool(ev.get("is_scam"))

    swaps = [act for act in (ev.get("actions") or []) if act.get("type") == "JettonSwap"]
    if not swaps:
    ...
    for act in swaps:
        ...
        ain_raw  = int((j.get("ton_in")  if tin.symbol  == "TON" else j.get("amount_in"))  or 0)
        aout_raw = int((j.get("ton_out") if tout.symbol == "TON" else j.get("amount_out")) or 0)
        legs.append(Leg(dex=dex, token_in=tin, token_out=tout,
                        amount_in_raw=ain_raw, amount_out_raw=aout_raw,
                        amount_in=tin.scale(ain_raw), amount_out=tout.scale(aout_raw)))
        if not a.wallet:
            a.wallet = _a(j.get("user_wallet")) or ""

    a.legs = legs; a.hops = len(legs); a.dex = legs[0].dex
    ...
    root = {"transaction": tr.get("transaction", {}), "children": tr.get("children") or []}
    ...
    for t in txs:
        ...
        if op in EXCESSES_OPS:
            refunds.append(Refund(op=op, amount_raw=v, amount=v / 1e9,
                                  to=wallet, kind="excesses (возврат газа)"))
    ...
    for t in txs:
        ...
        for m in (t.get("out_msgs") or []):
            if norm_addr(_a(m.get("destination"))) != wallet:
                continue
            v = int(m.get("value") or 0); op = (m.get("op_code") or "").lower()
            incoming += v
            if op in EXCESSES_OPS:
                refunds.append(Refund(op=op, amount_raw=v, amount=v / 1e9,
                                      to=wallet, kind="excesses (возврат газа)"))
    ...
    if a.gas_refunded > 0:
        ...
        if extra > 1e-9:
            refunds.append(Refund(op="0x474f86cf", amount_raw=int(extra * 1e9),
                                  amount=extra, to=wallet,
    ...
    if oracle is None:
        oracle = PriceOracle(api); api.prices = oracle
    def _px(meta: Meta) -> Price:
        return Price() if meta is None else oracle.at(meta.address, meta.symbol, a.timestamp)
    pin, pout = _px(a.token_in), _px(a.token_out)
    a.token_in.usd, a.token_out.usd = pin.usd, pout.usd
    if pin.usd is not None:  a.usd_in  = a.amount_in  * pin.usd
    if pout.usd is not None: a.usd_out = a.amount_out * pout.usd
    ...
    for l in a.legs:
        ...
        if pli.usd is not None: l.usd_in  = l.amount_in  * pli.usd
        if plo.usd is not None: l.usd_out = l.amount_out * plo.usd
    a.price_source = f"in={pin.source or '—'}({pin.ts}); out={pout.source or '—'}({pout.ts})"
...
def scan_wallet(wallet: str, limit: int = 20, api: TonAPI | None = None) -> list[SwapAnalysis]:
    ...
    for ev in (evs if isinstance(evs, list) else evs.get("events", [])):
        ...
        try:
            ...
            a.tx_hash = ev["event_id"]; out.append(a)
        except Exception:
            continue
    return out
...
_COLS = ("tx_hash,event_id,ts,wallet,dex,token_in_symbol,token_in_addr,token_in_decimals,"
...
class SwapStore:
    ...
    def save(self, a: SwapAnalysis) -> bool:
        d = a.to_dict()
        ti = d.get("token_in") or {}; to = d.get("token_out") or {}
        row = (
            d["tx_hash"], d["event_id"], d["timestamp"], d["wallet"], d["dex"],
            ti.get("symbol"), ti.get("address"), ti.get("decimals"),
            to.get("symbol"), to.get("address"), to.get("decimals"),
            d["amount_in"], d["amount_out"], d["rate"],
            d["usd_in"], d["usd_out"], d["usd_net"], d["usd_rate"],
            d["gas_bound"], d["gas_refunded"], d["gas_spent"],
            d["usd_gas_spent"], d["total_fees"], d["usd_total_fees"],
            d["net_ton_change"], json.dumps(d["net_jetton_change"], ensure_ascii=False),
            d["tx_count"], d["hops"], int(bool(d["success"])), int(bool(d["is_scam"])),
            d["price_source"], json.dumps(d["price_errors"], ensure_ascii=False),
            json.dumps(d["refunds"], ensure_ascii=False), json.dumps(d["legs"], ensure_ascii=False),
            int(time.time()),
        )
        q = (f"INSERT INTO swaps ({_COLS}) VALUES ({','.join('?' * len(row))}) "
        ...
        self.conn.execute(q, row); self.conn.commit()
        return True
    ...
    def history(self, wallet: str | None = None, limit: int = 50) -> list[dict]:
        if wallet:
            ...
            rows = self.conn.execute(
                "SELECT * FROM swaps ORDER BY ts DESC LIMIT ?", (limit,)).fetchall()
    ...
    def pnl(self, wallet: str | None = None, oracle=None) -> dict:
        ...
        rows = [dict(r) for r in self.conn.execute(
            f"SELECT * FROM swaps {where} ORDER BY ts ASC", params).fetchall()]
        now = int(time.time())
        vol = fees = gas = cost = 0.0; ton_net = 0.0
        bal: dict[str, dict] = {}
        for r in rows:
            vol += r["usd_in"] or 0.0; fees += r["usd_total_fees"] or 0.0
            # газ считаем по факту (total_fees трассы) — не зависит от наличия привязанного газа
            gas += (r["usd_total_fees"] if r["usd_total_fees"] is not None
                    else (r["usd_gas_spent"] or 0.0)); cost += r["usd_net"] or 0.0
            ...
            for lg in json.loads(r["legs"] or "[]"):
                for side in ("token_in", "token_out"):
                    ...
                    if not tm.get("symbol"):
                        continue
                    d = bal.setdefault(tm["symbol"], {"amount": 0.0, "addr": "", "decimals": 9})
                    if not d["addr"]:
                        d["addr"] = tm.get("address") or ""
                    d["decimals"] = tm.get("decimals") or d["decimals"]
        holdings = []; total = 0.0; errors = []
        def _value(sym, amount, addr):
            nonlocal total
            usd = None
            if oracle is not None:
                p = oracle.at(addr, sym, now); usd = p.usd
                if usd is None and p.error:
                    errors.append(sym)
            v = usd * amount if usd is not None else None
            if v is not None:
                total += v
            holdings.append({"symbol": sym, "amount": amount, "usd": usd, "usd_value": v})
        ...
        period = "—"
        if rows:
            period = (time.strftime("%Y-%m-%d", time.gmtime(rows[0]["ts"])) + " → " +
                      time.strftime("%Y-%m-%d", time.gmtime(rows[-1]["ts"])))
        return {"wallet": wallet or "все кошельки", "swaps": len(rows), "period": period,
                "volume_usd": vol, "fees_usd": fees, "gas_usd": gas, "swap_cost_usd": cost,
                "holdings": holdings,
...
def format_history(rows: list[dict]) -> str:
    ...
    out = [f"📚 История свапов: {len(rows)} записей", "",
    ...
    for r in rows:
        ...
        out.append(f"{t:19} {str(r['dex'] or '?'):8} {ain:>20} {'→':^3} {aout:>20} {net:>12}")
    return "\n".join(out)

def format_pnl(rep: dict) -> str:
    lines = ["📊 <b>PnL-отчёт (агрегат по свапам)</b>",
             f"👤 {rep['wallet']}",
...
def format_bot_message(a: SwapAnalysis) -> str:
    ts = time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime(a.timestamp)) if a.timestamp else "—"
    ...
    if a.rate:
        lines.append(f"📈 Курс: 1 {ti.symbol} = {_fmt(a.rate, 6)} {to.symbol}"
                     f"  (1 {to.symbol} = {_fmt(a.rate_inverse, 6)} {ti.symbol})")
    if a.usd_in is not None or a.usd_out is not None:
        def _usd(x): return "—" if x is None else f"${x:,.4f}" if abs(x) < 1 else f"${x:,.2f}"
        lines.append(f"💵 USD: отдано {_usd(a.usd_in)} → получено {_usd(a.usd_out)}")
        if a.usd_net is not None:
            lines.append(f"💵 Нетто USD: <b>{'+' if a.usd_net >= 0 else ''}{a.usd_net:,.4f}</b>")
    if a.price_errors:
        for k, v in a.price_errors.items():
            lines.append(f"⚠️ USD({k}): нет курса — {v}")
    if a.hops > 1:
        lines.append(f"🔀 Хопов: {a.hops} " + " → ".join(l.token_in.symbol + "/" + l.token_out.symbol
                                                        for l in a.legs))
    lines += ["", "⛽ <b>Газ</b>",
    ...
    lines += [f"• Всего газа по трассе: {_fmt(a.total_fees)} TON"
...
def main(argv: list[str]) -> int:
    args = argv[1:]
    fixture = None; watch = None; limit = 20; as_json = False; target = None; usd = True
    db_path = None; do_store = False; do_history = False; do_pnl = False
    i = 0
    while i < len(args):
        if args[i] == "--fixture-dir": fixture = args[i + 1]; i += 2
        elif args[i] == "--watch":     watch = args[i + 1]; i += 2
        elif args[i] == "--limit":     limit = int(args[i + 1]); i += 2
        elif args[i] == "--json":      as_json = True; i += 1
        elif args[i] == "--no-usd":    usd = False; i += 1
        elif args[i] == "--db":        db_path = args[i + 1]; i += 2
        elif args[i] == "--store":     do_store = True; i += 1
        elif args[i] == "--history":   do_history = True; i += 1
        elif args[i] == "--pnl":       do_pnl = True; i += 1
        else: target = args[i]; i += 1

    api = TonAPI()
    if not usd:
        class _NoPrice:
            def at(self, addr, symbol, ts): return Price()
        api.prices = _NoPrice()
    oracle = getattr(api, "prices", None)
    if oracle is None and usd:
        oracle = PriceOracle(api); api.prices = oracle

    store = SwapStore(db_path) if (db_path or do_store or do_history or do_pnl) else None

    if do_history:
        rows = store.history(wallet=target, limit=limit)
        print(json.dumps(rows, ensure_ascii=False, indent=1) if as_json else format_history(rows))
        return 0
    if do_pnl:
        rep = store.pnl(wallet=target, oracle=oracle)
        print(json.dumps(rep, ensure_ascii=False, indent=1) if as_json else format_pnl(rep))
        return 0
    ...
    try:
        if fixture:
            ...
            a = normalize(ev, tr, api); a.tx_hash = ev.get("event_id", "")
            res = [a]
        elif watch:
            ...
            if not re.fullmatch(r"[0-9a-fA-F]{64}", target or ""):
                print(f"Ошибка: '{target}' не похож на хеш транзакции TON (нужно 64 hex-символа).\n"
                      f"Для адреса кошелька используйте: --watch <адрес>", file=sys.stderr)
                return 2
            res = [parse_swap(target, api=api)]
        else:
            print(__doc__); return 2
    except urllib.error.HTTPError as e:
        print(f"Ошибка: TonAPI ответил HTTP {e.code} для '{target or watch}'. "
              f"Проверьте хеш/адрес или повторите позже.", file=sys.stderr)
        return 1
    ...
    for a in res:
        if store is not None and (do_store or db_path):
            store.save(a)
        print(json.dumps(a.to_dict(), ensure_ascii=False, indent=1) if as_json
