"""
BrainFusion v3 — Quantum Intelligence Engine для TON/USDT.
...
Объединяет 8 источников сигналов в один консенсусный «организм»:
  ...
  5. Neural Prophet (attention-based pattern matching + MLP)
  6. Market Sentiment (Fear&Greed, Order Flow, Divergence)
  7. Swarm Intelligence (16 evolving agents)
  8. XAI Trust Score (объяснимый ИИ + фильтр надёжности)

Возможности:
  • Консенсусный сигнал с динамическими весами 8 источников
  • Детектор скальпинг-окна (RANGING/SQUEEZE → быстрые сделки)
  • Детектор памп-ускорителя (PUMP/BREAKOUT → увеличенная позиция)
  • «Пропуск подтверждения» — когда все источники согласны ≥78%
  • Анализ баланса кошелька — TON + USDT в TON эквиваленте
  • Журнал решений с самооценкой + XAI аудит
  • v7: Quantum Optimizer — автооптимизация параметров сетки
...
Использование:
    ...
    bf.update_v7(prophet_result, sentiment_result, swarm_result)
...
_W_AI = 0.40  # ML ансамбль (уменьшен для v7)
_W_TA = 0.12  # Технический анализ
_W_ADVISOR = 0.06  # LLM советник
_W_SCANNER = 0.05  # Market Scanner
# v7: Quantum Intelligence Suite
_W_PROPHET = 0.15  # Neural Prophet — предсказание цен
_W_SENTIMENT = 0.10  # Market Sentiment — Fear&Greed
_W_SWARM = 0.08  # Swarm Intelligence — рой агентов
_W_XAI_TRUST = 0.04  # XAI Trust Score — фильтр надёжности
...
class BrainFusion:
    ...
    def __init__(self):
        ...
        # v7: Quantum Intelligence Suite state
        self._v7_prophet: dict = {}
        self._v7_sentiment: dict = {}
        self._v7_swarm: dict = {}
        self._v7_xai_trust: float = 0.0
        self._v7_updated_at: float = 0.0
        self._v7_ttl: float = 120.0  # 2 мин TTL
    ...
    def update_wallet(
        ...
        with self._lock:
            ...
            _usdt_pos_threshold = getattr(Config, "USDT_MIN_POSITION_HOLD", 10)
            w.has_position = grinch_bal > _usdt_pos_threshold
            w.updated_at = time.time()

    def update_v7(
        self, prophet: dict = None, sentiment: dict = None, swarm: dict = None
    ):
        """v3: Обновить состояние Quantum Intelligence Suite."""
        with self._lock:
            self._v7_prophet = prophet or {}
            self._v7_sentiment = sentiment or {}
            self._v7_swarm = swarm or {}
            # XAI trust score
            if prophet:
                self._v7_xai_trust = prophet.get("trust_score", 0)
            elif sentiment:
                self._v7_xai_trust = sentiment.get("conviction", 0)
            else:
                self._v7_xai_trust = 0
            self._v7_updated_at = time.time()
    ...
    def _compute_fusion(self) -> FusionSignal:
        ...
        prophet_conf_n = 0.0
        sentiment_num = 0.0
        sentiment_conf_n = 0.0
        swarm_num = 0.0
        swarm_conf_n = 0.0
        xai_trust = self._v7_xai_trust

        if v7_fresh:
            # Prophet
            p_sig = self._v7_prophet.get("signal", "HOLD")
            prophet_num = _sig_to_num(p_sig)
            prophet_conf_n = self._v7_prophet.get("confidence", 0) / 100.0

            # Sentiment
            s_score = self._v7_sentiment.get("sentiment_score", 0)
            sentiment_num = 1.0 if s_score > 0.3 else (-1.0 if s_score < -0.3 else 0.0)
            sentiment_conf_n = self._v7_sentiment.get("conviction", 0)

            # Swarm
            swarm_num = _sig_to_num(self._v7_swarm.get("consensus", "HOLD"))
            swarm_conf_n = self._v7_swarm.get("confidence", 0) / 100.0
        ...
        # ── Взвешенный консенсус (8 источников) ─────────────────────────
        ...
        w_prophet = _W_PROPHET * prophet_conf_n if v7_fresh else 0.0
        w_sentiment = _W_SENTIMENT * sentiment_conf_n if v7_fresh else 0.0
        w_swarm = _W_SWARM * swarm_conf_n if v7_fresh else 0.0
        w_xai = _W_XAI_TRUST * xai_trust if v7_fresh else 0.0

        w_sum = (
            w_ai + w_ta + w_adv + w_scanner + w_prophet + w_sentiment + w_swarm + w_xai
        )
        ...
        weighted = (
            ...
            + w_ta * ta_num
            + w_adv * adv_num
            + w_scanner * scanner_num
            + w_prophet * prophet_num
            + w_sentiment * sentiment_num
            + w_swarm * swarm_num
            + w_xai * (1.0 if prophet_num > 0 else (-1.0 if prophet_num < 0 else 0.0))
        ) / w_sum
        ...
        all_agree = (
            ...
            and adv_agrees
            and prophet_agrees
            and sentiment_agrees
            and swarm_agrees
        )

        # Для пропуска нужно: AI свежий + уверенность ≥ порога + все согласны + EV OK + XAI trust
        fs.skip_confirmation = (
            ...
            and self._ai.ev_ok
            and fs.action != "HOLD"
            and (not v7_fresh or xai_trust >= 0.5)  # v7: требуем доверие ≥50%
        )
        ...
        fs.sources = {
            ...
            "prophet": {
                "signal": self._v7_prophet.get("signal", "HOLD"),
                "conf": round(self._v7_prophet.get("confidence", 0), 1),
                "fresh": v7_fresh,
            },
            "sentiment": {
                "signal": (
                    "BULLISH"
                    if sentiment_num > 0
                    else "BEARISH" if sentiment_num < 0 else "NEUTRAL"
                ),
                "conf": round(sentiment_conf_n * 100, 1),
                "fresh": v7_fresh,
            },
            "swarm": {
                "signal": self._v7_swarm.get("consensus", "HOLD"),
                "conf": round(swarm_conf_n * 100, 1),
                "fresh": v7_fresh,
            },
            "xai_trust": {
                "score": round(xai_trust, 3),
                "fresh": v7_fresh,
            },
        }
        ...
        if v7_fresh:
            parts.append(
                f"PROPHET={self._v7_prophet.get('signal', 'HOLD')}"
                f"({self._v7_prophet.get('confidence', 0):.0f}%)"
            )
            parts.append(
                f"SENTIMENT={sentiment_num:+.1f}"
                f"(F&G={self._v7_sentiment.get('fear_greed', {}).get('label', 'N/A')})"
            )
            parts.append(
                f"SWARM={self._v7_swarm.get('consensus', 'HOLD')}"
                f"({swarm_conf_n*100:.0f}%)"
            )
        mode = []
        if is_scalp:
    ...
    def get_state(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "v7": {
                    "prophet": (
                        self._v7_prophet.get("signal", "HOLD")
                        if self._v7_prophet
                        else "HOLD"
                    ),
                    "sentiment_score": (
                        round(self._v7_sentiment.get("sentiment_score", 0), 3)
                        if self._v7_sentiment
                        else 0
                    ),
                    "swarm": (
                        self._v7_swarm.get("consensus", "HOLD")
                        if self._v7_swarm
                        else "HOLD"
                    ),
                    "xai_trust": round(self._v7_xai_trust, 3),
                    "fresh": (time.time() - self._v7_updated_at) < self._v7_ttl,
                },
                "scalp_streak": self._scalp_streak,
