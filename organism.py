...
class Organism:
    ...
    def __init__(self):
        ...
        # ── v2: Quantum Mood Integration ────────────────────────────────────
        self._quantum_mood_bias = 0.0
        ...
        self._start_dreams()
        logger.info("🧬 Organism v2.0 пробудился — 7 систем + Quantum Mood активны")
    ...
    def _update_mood_from_ai(self, ai: Dict) -> None:
        ...
        # v2: Quantum Mood — учитываем market sentiment
        sentiment_score = ai.get("sentiment_score")
        if sentiment_score is not None:
            s = float(sentiment_score)
            self._quantum_mood_bias = self._quantum_mood_bias * 0.8 + s * 0.2
            if self._quantum_mood_bias > 0.2:
                self.greed = min(1.0, self.greed + self._quantum_mood_bias * 0.08)
            elif self._quantum_mood_bias < -0.2:
                self.fear = min(1.0, self.fear + abs(self._quantum_mood_bias) * 0.08)
