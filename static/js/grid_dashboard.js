...
async function updateBalance() {
    try {
        ...
        if (tonEl) tonEl.textContent = d.ton.amount.toLocaleString('ru-RU', {minimumFractionDigits: 2, maximumFractionDigits: 4});
        
        const tonUsd = document.getElementById('bal-ton-usd');
        if (tonUsd) tonUsd.textContent = d.ton.usd.toLocaleString('ru-RU', {minimumFractionDigits: 2, maximumFractionDigits: 2});
        
        const priceTon = document.getElementById('price-ton');
        if (priceTon) priceTon.textContent = '$' + d.ton.price.toFixed(2);
        
        const tokenEl = document.getElementById('bal-token');
        if (tokenEl) tokenEl.textContent = Math.floor(d.token.amount).toLocaleString('ru-RU');
        
        const tokenUsd = document.getElementById('bal-token-usd');
        if (tokenUsd) tokenUsd.textContent = d.token.usd.toLocaleString('ru-RU', {minimumFractionDigits: 2, maximumFractionDigits: 2});
        
        const tokenTon = document.getElementById('bal-token-ton');
        if (tokenTon) tokenTon.textContent = d.token.price_ton.toFixed(4);
        
        const priceToken = document.getElementById('price-token');
        if (priceToken) priceToken.textContent = '$' + d.token.price.toFixed(6);
        
        const tName = document.getElementById('token-name');
        if (tName) tName.textContent = d.token.symbol;
        
        const tName2 = document.getElementById('token-name-2');
        if (tName2) tName2.textContent = d.token.symbol;
    } catch(e) {}
}
