...
class BrainState:
    ...
    # BrainFusion consensus + LLM news + trade feedback
    fusion_action: str = "HOLD"
    fusion_confidence: float = 0.0
    fusion_reasoning: str = ""
    llm_news_score: Optional[float] = None
    llm_news_summary: str = ""
    last_trade_pnl_ton: float = 0.0
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "fusion": {
                "action": self.fusion_action,
                "confidence": round(self.fusion_confidence, 1),
                "reasoning": self.fusion_reasoning,
            },
            "llm_news": {
                "score": self.llm_news_score,
                "summary": self.llm_news_summary,
            },
            "kimi": {
...
class QuantumBrain:
    ...
    def _llm_news_payload(self) -> dict:
        """Свежий LLM-новостной сентимент для контекста советника."""
        try:
            from ai.sentiment import get_llm_news
            ...
            payload = {
                ...
                "fresh": llm.get("fresh", False),
            }
            if llm.get("score") is not None:
                self.state.llm_news_score = float(llm["score"])
                self.state.llm_news_summary = str(llm.get("summary", ""))[:160]
            return payload
        except Exception:
            return {"score": None, "fresh": False}
    ...
    def _run_fusion(self):
        """BrainFusion: сводный консенсус AI+TA+LLM+v7 с обратной связью."""
        try:
            bf.update_ai(
                {
            ...
            bf.update_ta(
                {
                    "signal": self.state.sentiment_signal,
                    "entry_quality": "B",
                    "entry_score": 50,
                    "rsi": 50,
                }
            ...
            bf.brain.update_v7(
                prophet={
                    "signal": self.state.prophet_signal,
                    "confidence": self.state.prophet_confidence,
                    "trust_score": self.state.xai_trust,
                },
                sentiment={
                    "signal": self.state.sentiment_signal,
                    "conviction": self.state.sentiment_fg / 100.0,
                },
                swarm={"consensus": self.state.swarm_consensus, "confidence": 0.5},
            )
            if self.state.kimi_ready:
                bf.update_advisor(
                    verdict=self.state.kimi_signal,
                    confidence=self.state.kimi_confidence / 100.0,
                    regime=self.state.regime,
                    advice=self.state.kimi_reason,
                    next_check_min=10,
                )
            fs = bf.get_fusion_signal()
            self.state.fusion_action = fs.action
            self.state.fusion_confidence = fs.consensus_conf
            self.state.fusion_reasoning = fs.reasoning
        except Exception as e:
            log.warning("[QuantumBrain] BrainFusion error: %s", e)

    def notify_trade_closed(self, pnl_ton: float, was_scalp: bool = False):
        """Обратная связь о закрытой сделке → BrainFusion (пересчёт весов)."""
        try:
