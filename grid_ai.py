...
# ═══════════════════════════════════════════════════════════════════════════════
# v7: Quantum Intelligence Suite imports
# ═══════════════════════════════════════════════════════════════════════════════
try:
    from ai.neural_prophet import predict_price
    from ai.quantum_optimizer import get_recommended_params
    from ai.sentiment import analyze_sentiment
    from ai.swarm import analyze_swarm
    from ai.xai_explainer import explain

    _V7_AVAILABLE = True
except Exception as _v7_err:
    _V7_AVAILABLE = False
    log.debug("[GridAI v7] Quantum modules not available: %s", _v7_err)
...
class GridAI:
    ...
    def __init__(self):
        ...
        # ════════════════════════════════════════════════════════════════
        self._v7_enabled = _V7_AVAILABLE
        self._last_v7_analysis: dict = {}
        self._last_candles: list = []
        self._v7_tick_count = 0
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # v7: Quantum Intelligence Suite — unified signal
    # ══════════════════════════════════════════════════════════════════════════

    def get_quantum_signal(self, current_price: float, candles: list) -> dict:
        """v7: Единый квантовый сигнал от всех ИИ-модулей.

        Объединяет:
          • Neural Prophet (pattern matching + attention + MLP)
          • Market Sentiment (Fear&Greed, Order Flow, Divergence)
          • Swarm Intelligence (16 evolving agents)
          • Quantum Optimizer (grid parameter optimization)
          • XAI Explainer (trust scores + counterfactuals)

        Возвращает консенсусный сигнал с объяснением.
        """
        if not self._v7_enabled or len(candles) < 30:
            return {"signal": "HOLD", "confidence": 0.0, "v7_active": False}

        self._last_candles = candles
        self._v7_tick_count += 1

        try:
            # 1. Neural Prophet — предсказание цены
            prophet = predict_price(current_price, candles)

            # 2. Sentiment — настроения рынка
            sentiment = analyze_sentiment(candles)

            # 3. Swarm — консенсус роя
            swarm = analyze_swarm(candles)

            # 4. Quantum Optimizer — оптимальные параметры (раз в час)
            opt_params = get_recommended_params(candles)

            # 5. Fusion: взвешенное голосование
            signals = {
                "ml": {
                    "signal": prophet.get("signal", "HOLD"),
                    "conf": prophet.get("confidence", 0),
                },
                "sentiment": {
                    "signal": (
                        "BUY"
                    ...
                    "conf": sentiment.get("conviction", 0) * 100,
                },
                "swarm": {
                    "signal": swarm.get("consensus", "HOLD"),
                    "conf": swarm.get("confidence", 0),
                },
            }

            # Веса источников (динамические: Prophet > Swarm > Sentiment)
            weights = {"ml": 0.40, "sentiment": 0.25, "swarm": 0.35}
            vote = {"BUY": 0.0, "SELL": 0.0, "HOLD": 0.0}

            for source, data in signals.items():
                w = weights.get(source, 0.2)
                vote[data["signal"]] += w * data["conf"]
            ...
            conf = vote[consensus] * 100

            # Сбор признаков для XAI
            feat = (
                self._make_features(
                    candles[-1].get("atr_pct", 3.5) if candles else 3.5,
                    self._last_regime or "UNKNOWN",
                )
                if hasattr(self, "_make_features")
                else {}
            )
            features_dict = (
                {f"feat_{i}": v for i, v in enumerate(feat)}
            ...
            xai_result = explain(
                ...
                swarm=swarm,
                prophet=prophet,
                ai_conf=signals["ml"]["conf"],
                sentiment_conf=signals["sentiment"]["conf"],
                swarm_conf=signals["swarm"]["conf"],
                volatility=candles[-1].get("atr_pct", 3.5) if candles else 3.5,
            )
            ...
            result = {
                ...
                "vote_distribution": {k: round(v, 4) for k, v in vote.items()},
                "explanation": xai_result.get("explanation", ""),
                "trust_score": xai_result.get("trust_score", 0),
                "trust_label": xai_result.get("trust_label", ""),
                "top_features": xai_result.get("top_features", []),
                "counterfactuals": xai_result.get("counterfactuals", []),
            }

            self._last_v7_analysis = result
            log.info(
                "[GridAI v7] QuantumSignal: %s (%.1f%%) trust=%.2f (%s)",
                consensus,
                conf,
                result["trust_score"],
                result["trust_label"],
            ...
            log.warning("[GridAI v7] Quantum signal error: %s", e)
            return {
                "signal": "HOLD",
                "confidence": 0.0,
                "v7_active": False,
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "v7_active": self._v7_enabled,
                "v7_tick_count": self._v7_tick_count,
                "v7_last_analysis": self._last_v7_analysis,
            }
