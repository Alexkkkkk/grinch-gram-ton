...
def api_balance():
    """Баланс кошелька для дашборда."""
    try:
        from dedust_client import dedust
        ton = dedust.get_ton_balance() if hasattr(dedust, "get_ton_balance") else 0.0
        token = dedust.get_token_balance() if hasattr(dedust, "get_token_balance") else 0.0
        price_ton = price_feed.get("TON") or 0
        token_symbol = (Config.SYMBOL or "GRINCH/TON").split("/")[0]
        price_token = price_feed.get(token_symbol) or 0
        price_token_ton = price_feed.get_token_ton_price() or 0
        
        return jsonify({
            "ok": True,
            "ton": {"amount": round(ton, 4), "usd": round(ton * price_ton, 2), "price": round(price_ton, 4)},
            "token": {
                "symbol": token_symbol,
                "amount": round(token, 2),
                "usd": round(token * price_token, 2),
                "price": round(price_token, 6),
                "price_ton": round(price_token_ton, 6)
            }
        })
    except Exception as e:
...
def api_history():
    """История PnL для графиков grid-дашборда."""
    try:
        from grid_db import get_grid_stats, get_pnl_history

        stats = get_grid_stats()
        pnl = get_pnl_history(hours=int(request.args.get("hours", 24)))
        return jsonify(
            {
                "ok": True,
                "stats": stats,
                "pnl": [
                    {
                        "total_profit_usdt": p.get("profit", 0),
        ...
        logger.error("api_history error: %s", e, exc_info=True)
        return jsonify({"ok": False, "error": "Внутренняя ошибка сервера"}), 500
