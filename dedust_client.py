"""
DeDust DEX клиент для реальной торговли GRAM/USDT в блокчейне TON.
GRAM = нативный TON (переименован в 2026).
...
def _fetch_balance_and_update(force: bool, now: float) -> dict:
    ...
    # USDT decimals = 6 (не 9 как у USDT)
    if not hit_429:
        try:
            r = _HTTP.get(
...
class DedustClient:
    ...
    async def _usdt_balance_nano(self, provider, addr) -> int:
        ...
        try:
            usdt_root = JettonRoot.create_from_address(Config.TOKEN_ADDRESS)
    ...
    def _usdt_address(self) -> Address:
        ...
        return Address(Config.TOKEN_ADDRESS)
    ...
    async def _get_pool(self, provider):
        ...
        pool_addr = (getattr(Config, "USDT_POOL_ADDRESS", "") or "").strip()
        if pool_addr:
    ...
    def get_price_ton_per_usdt(self) -> Optional[float]:
        ...
        try:
            ...
            # reserves[0] = TON резерв (нано), reserves[1] = USDT резерв (нано)
    ...
    def _min_out_buy_usdt(self, ton_amount: float):
        ...
        return int(min_usdt * (10**9)), expected_usdt
    ...
    async def _buy_async(self, ton_amount: float) -> dict:
        ...
        try:
            ...
            min_delta = int(expected_usdt * 0.5 * (10**9))
            ...
            return {
                ...
                "min_usdt_out": round(min_out_nano / (10**9), 6),
                "expected_usdt": round(expected_usdt, 6),
                "usdt_received": round((confirmed - baseline_nano) / (10**9), 6),
    ...
    async def _sell_async(self, usdt_amount: float, min_net_ton: float = None) -> dict:
        ...
        try:
            ...
            amount_nano = min(int(usdt_amount * (10**9)), baseline_nano)
            if amount_nano <= 0:
                return {
                    "ok": False,
                    "side": "sell",
            ...
            log.info(
                f"[DeDust] SELL {amount_nano/1e9:.6f} USDT (requested={usdt_amount:.6f}, on-chain={baseline_nano/1e9:.6f})"
            )
            ...
            return {
                ...
                "usdt_sold": round((baseline_nano - confirmed) / (10**9), 6),
                "slippage_pct": Config.SLIPPAGE_PCT,
            }
        finally:
            await provider.close_all()
