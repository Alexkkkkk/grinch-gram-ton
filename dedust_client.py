...
class DedustClient:
    ...
    _JETTON_XFER_OP = 0x0F8A7EA5  # стандартный TEP-74 jetton transfer
    # Параметры подтверждённых legacy DeDust-транзакций:
    # buy: 0.25 TON на swap + минимум 0.25 TON на Native Vault;
    # sell: 0.30 TON на jetton-wallet, из них 0.25 TON forward в пул.
    _BUY_MIN_GAS_TON = 0.25
    _SELL_ATTACHED_TON = 0.30
    _SELL_FORWARD_TON = 0.25
    ...
    def _build_sell_transfer_body(
        ...
        """Build a TEP-74 transfer to the DeDust Jetton Vault.

        The pool address belongs in the forward swap payload, while the
        Jetton Transfer destination must be the Vault's jetton-wallet owner.
        This is the route used by the confirmed legacy DeDust transactions.
        """
        ...
        return (
            ...
            .store_address(vault_addr)  # destination = Jetton Vault owner
    ...
    async def _sell_async(self, usdt_amount: float, min_net_ton: float = None) -> dict:
        ...
        try:
            ...
            # Legacy DeDust flow: transfer USDT to the jetton Vault; the Vault
            # then forwards the swap message to the pool.
            jetton_vault_addr = await Factory.get_vault_address(
                Asset.jetton(self._usdt_address()), provider
            )

            # ── Газ для sell ────────────────────────────────────────────────
            # This matches the successful legacy DeDust trace: 0.30 TON is
            # attached to the jetton-wallet transfer and 0.25 TON is forwarded
            # to the pool. Network fees consume the small difference; excesses
            # are returned to the wallet.
            gas_nano = int(self._SELL_ATTACHED_TON * TON)
            fwd_nano = int(self._SELL_FORWARD_TON * TON)
            ...
            # USDT уходит стандартным jetton-transfer'ом в wallet DeDust
            # Jetton Vault. Vault маршрутизирует токены в пул по forward-payload.
            await wallet.transfer(
