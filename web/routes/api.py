...
def _grid_request_settings(data, price):
    ...
    if os.getenv("GRID_SELL_AS_TON", "0").strip() == "1":
        # TON/USDT cycle still honors the configured initial split.
        # Later BUY levels may also be added dynamically from settled SELL proceeds.
        configured_sell = max(1, int(getattr(Config.GRID, "sell_levels", grid_count)))
        configured_buy = max(0, int(getattr(Config.GRID, "buy_levels", 0)))
        sell_levels = min(grid_count, configured_sell)
        buy_levels = min(max(0, grid_count - sell_levels), configured_buy)
    else:
