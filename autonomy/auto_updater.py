...
AUTO_UPDATE_ENABLED = __import__("os").getenv("AUTO_UPDATE_ENABLED", "").lower() in (
    "1",
    "true",
    "yes",
)
...
class AutoUpdater:
    ...
    def apply_update(self) -> Dict:
        ...
        backup_dir = (
            self.repo_path.parent / f"backups/bot-{datetime.utcnow():%Y%m%d-%H%M%S}"
        )
        steps = []
        try:
        ...
        record = {
            "timestamp": datetime.utcnow().isoformat(),
            "results": steps,
            "success": all(r["status"] == "success" for r in steps),
        }
